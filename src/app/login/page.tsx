...
import { ChevronRight, Store, Loader2, ArrowLeft, ShieldCheck, MapPin, Phone, User } from 'lucide-react';
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      const authTimeoutPromise = new Promise<never>((_, reject) => 
        setTimeout(() => reject(new Error('Google Sign-in timed out. Please check if browser popups are blocked or try again.')), 15000)
      );
      ...
      const queryTimeoutPromise = new Promise<any>((_, reject) => 
        setTimeout(() => reject(new Error('Database connection timed out. Please check your internet connection or verify that firestore.googleapis.com is not blocked by your firewall/ad-blocker.')), 8000)
      );
  ...
  const handleRegisterSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const timeoutPromise = new Promise<any>((_, reject) => 
        setTimeout(() => reject(new Error('Database write connection timed out. Please verify that your internet is active, or check if firestore.googleapis.com is blocked by an ad-blocker or firewall.')), 8000)
      );
            ...
            <div className="space-y-6 animate-fade-in">
              <div className="space-y-2">
                ...
                <p className="text-sm text-foreground/50 leading-relaxed font-medium">
                  Hi <strong className="text-foreground font-black">{googleUser?.displayName}</strong>, please provide your phone number and address to finish setting up your account.
                </p>
              </div>
